...
# import the full PF dataset

data = xr.open_dataset(r'japan_ECMF_PF.2017-2019_subset.nc')

# plot the runoff data
#data.info()

# conver the runoff to the same units as the ERA5 data
# we will do this by dividing by 1000
data['ro'] = data['ro']/1000

# now to get runoff rate per day from this data
# we need to subtract the runoff from the previous day
# first we need to group the data by the time coordinate
# and take the mean
data = data.groupby('time').mean(dim='time')

data['ro'] = data['ro'].diff(dim='time')

# plot the runoff data
#data['ro'].plot()

# print the statistics of the runoff data
# mean, standard deviation, minimum, maximum
print(data['ro'].mean().values)
print(data['ro'].std().values)
print(data['ro'].min().values)
print(data['ro'].max().values)
...
data_2017 = data.sel(time=slice('2017-06-01', '2017-09-14'))
...
# plot the 1:1 line
plt.plot([0, 0.05], [0, 0.05], color='black', linestyle='--')
# set the axis labels
...
data_2017['ro_calibrated'] = data_2017['ro'].values.flatten() * c1 + c0
...
# include the standard deviations
plt.axvline(data_2017['ro_calibrated'].mean().values + data_2017['ro_calibrated'].std().values, color='purple', linestyle='--')
plt.axvline(data_2017['ro_calibrated'].mean().values - data_2017['ro_calibrated'].std().values, color='purple', linestyle='--')
